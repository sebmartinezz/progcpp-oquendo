...
double dcentral_with_v (double x, double v, double h, fptrv f) //parametro variable v
{
...
double gauss3 (double a, double b, fptr f){
  ...
  double aux0 = x0*(b-a)/2.0 + (b+a)/2.0;
  double aux1 = x1*(b-a)/2.0 + (b+a)/2.0;
  double aux2 = x2*(b-a)/2.0 + (b+a)/2.0;

  return (w0*f(aux0) + w1*f(aux1) + w2*f(aux2))*(b-a)/2.0;


//sig: arreglo para simplificar gauss11
}
...
{
  //parámetro variable v

  if (std::fabs(f(x0, v))<=tol){
    return x0;
  }
  ...
  for (long niter=0; niter <= nmax; niter++){
    ...
    double aux0 = f(x0, v)/d(x0, v, h, f);
    x1 = x0 - aux0;

    if (std::fabs(f(x1, v))<=tol){
      return x1;
    }else{
      x0=x1;
    }
