...
int main(int argc, char **argv) {
  ...
  // Boundary conditions
  Boundary bc(2.345, 0.0, 0.0, 0.0, 0.99); // RMAX, CX, CY, CZ, EN
  ...
  //create vtk exporter
  VTKExporter vtkexporter("output");
  vtkexporter.reset_counter();
  ...
  for(int ii = 1; ii < niter; ++ii) {
    ...
    if (ii%10 == 0){
        vtkexporter.write_frame(bodies, p["DT"]*ii);
    }
  }
...
void initial_conditions(std::vector<Particle> & particles)
{
  particles[0].R[2] = 0.1;  // z is upwards, x to the right
